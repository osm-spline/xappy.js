...
var createNodeSubQuery = function (xapiRequestObject) {
    ...
    function createRelationTagCondition() {
        var index = query.values.length + 1;
        var parts = [];
        var tagCondition;
    ...
    function createNodeTagCondition() {
        var index = query.values.length + 1;
        var parts = [];
        var tagCondition;
        _.each(xapiRequestObject.tag.key, function (key) {
            _.each(xapiRequestObject.tag.value, function (value) {
                query.values.push(key);
                query.values.push(value);
                parts.push('nodes.tags @> hstore($' + (t++) + ', $' + (index++) + ')');
            });
        });
        return parts.join(' OR ');
    }
    
    function createWayTagCondition() {
        var index = query.values.length + 1;
        var parts = [];
        var tagCondition;
        _.each(xapiRequestObject.tag.key, function (key) {
    ...
    if (xapiRequestObject.tag) {
        ...
        query1 += tagRelationCondition;
        query2 += tagRelationCondition;
        //query3 += ...
    }
...
var createRelationSubQuery = function (xapiRequestObject) {
    ...
    var relationColumns =
        ...
        'relations.tstamp, relations.changeset_id, hstore_to_array(relations.tags) ' +
        'AS tags, users.name AS user_name, ' +
