...
var createQueryPlan = function(xapiQueryObject) {
    ...
    if (sql_where.length === 0) {
        if(xapiQueryObject.predicate === 'way') {
                ...
                sql_predicate = 'SELECT nodes.id, nodes.version, nodes.user_id, users.name AS user_name, nodes.tstamp, nodes.changeset_id, hstore_to_array(tags), X(geom) AS lat, Y(geom) AS lon FROM nodes, users WHERE nodes.id NOT IN (SELECT node_id FROM way_nodes);';
            }

        else if(xapiQueryObject.predicate === 'tag') {
                sql_predicate = 'SELECT DISTINCT nodes.id, nodes.version, nodes.user_id, users.name AS user_name, nodes.tstamp, nodes.changeset_id, hstore_to_array(tags), X(geom) AS lat, Y(geom) AS lon FROM nodes, users WHERE nodes.user_id = users.id AND array_length(avals(nodes.tags), 1) > 0;';
            }
