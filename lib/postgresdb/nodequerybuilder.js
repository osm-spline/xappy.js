...
var createQueryPlan = function (xapiQueryObject) {
    ...
    sql_select =
        'SELECT nodes.id, nodes.version, nodes.user_id, users.name ' +
        'AS user_name, nodes.tstamp, nodes.changeset_id, hstore_to_array(nodes.tags) ' +
        'AS tags, X(nodes.geom) AS lat, Y(nodes.geom) AS lon FROM nodes, users ';
    ...
    if (sql_where.length === 0) {
        if (xapiQueryObject.predicate === 'way') {
            sql_predicate =
                'SELECT DISTINCT nodes.id, nodes.version, nodes.user_id, users.name ' +
                'AS user_name, nodes.tstamp, nodes.changeset_id, hstore_to_array(tags), X(geom) ' +
                'AS lat, Y(geom) AS lon FROM nodes, way_nodes, users ' +
                'WHERE nodes.id = way_nodes.node_id AND nodes.user_id = users.id;';
        } else if (xapiQueryObject.predicate === 'not(way)') {
            sql_predicate =
                'SELECT nodes.id, nodes.version, nodes.user_id, users.name ' +
                'AS user_name, nodes.tstamp, nodes.changeset_id, hstore_to_array(tags), X(geom) ' +
                'AS lat, Y(geom) AS lon FROM nodes, users ' +
            ...
            sql_predicate =
                'SELECT nodes.id, nodes.version, nodes.user_id, users.name ' +
                'AS user_name, nodes.tstamp, nodes.changeset_id, hstore_to_array(nodes.tags) ' +
                'AS tags, Y(nodes.geom) AS lat, X(nodes.geom) AS lon FROM nodes, users ' +
