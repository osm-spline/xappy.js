...
var createNodeSubQuery = function (xapiRequestObject) {
    ...
    function createSpacialCondition() {
        ...
        var spacialCondition =
            'ST_Crosses(st_setsrid(ways.linestring,4326),st_setsrid(st_makebox2d(' +
    ...
    function createSubSelect() {
        ...
        if (xapiRequestObject.bbox) {
            subSelect += ' AND (' + createSpacialCondition() + ')';
        }
        if (xapiRequestObject.tag) {
            subSelect += ' AND (' + createTagCondition() + ')';
        }
        return subSelect;
    }

    query.text =
        'SELECT ' + columns + ' FROM nodes, users, (' + createSubSelect() + ') ' +
        'AS nodesOfWays ' +
        'WHERE nodes.user_id = users.id ' +
        'AND nodesOfWays.node_id = nodes.id;';
    return query;
};
...
var createWaySubQuery = function (xapiRequestObject) {
    ...
    query.text =
        'SELECT ' + columns + ' FROM ways, users WHERE ways.user_id = users.id';
