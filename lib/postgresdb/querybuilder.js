	...
	if(xapiQueryObject.object === 'node') {
		...
		if (xapiQueryObject.bbox !== undefined) {
			if (sql_where.length > 0) {
				sql_where =  "(" + sql_where +")" + " AND (linestring && st_setsrid(st_makebox2d(st_setsrid(st_makepoint($1, $2),4326)),st_srid(st_makepoint($3, $4),4326)),4326))" 		
			}else{
				sql_where = "WHERE (linestring && st_setsrid(st_makebox2d(st_setsrid(st_makepoint($1, $2),4326)),st_srid(st_makepoint($3, $4),4326)),4326))" 	
			}
