...
QueryBuilder.prototype.createQueryPlan = function(xapiQueryObject) {
    ...
    if(xapiQueryObject.object === 'node') {
        ...
        if (xapiQueryObject.tag !== undefined) {
            var tags = xapiQueryObject.tag;
            var keys = tags.key;
            var values = tags.value;

            var foo = keys.concat(values);
            key_values = key_values.concat(foo);

            for ( var k= 0; k<xapiQueryObject.tag.key.length;k++) { 
                for ( var l= 0; l<xapiQueryObject.tag.value.length;l++) {
                    if (k!==0||l!==0) {
                        sql_where =  sql_where + " OR nodes.tags @> hstore($" + (key_values_count+ k) +",$" + (key_values_count+ l + xapiQueryObject.tag.key.length) +")";      
                    }else{
                        if(sql_where.length !==0){
                            sql_where = sql_where + " AND (nodes.tags @> hstore($" + (key_values_count+ k) +",$" + ( (key_values_count+ l + xapiQueryObject.tag.key.length)) +")";  
                        }else{      
                            sql_where = "WHERE (nodes.tags @> hstore($" + (key_values_count+ k) +",$" + ( (key_values_count+ l + xapiQueryObject.tag.key.length)) +")";     
                        }                   
                    }
                }
            }
    ...
    if(xapiQueryObject.object === 'way') {
        ...
        if (xapiQueryObject.tag !== undefined) {
            for ( var i2= 0; i2<xapiQueryObject.tag.key.length;i2++) { 
                key_values = key_values.concat([xapiQueryObject.tag.key[i2]]);
            }
            for ( var r= 0; r<xapiQueryObject.tag.value.length;r++) {   
                key_values = key_values.concat([xapiQueryObject.tag.value[r]]);
            }
