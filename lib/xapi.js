...
function Xapi(config) {
    ...
    var handler = httpHandler(parse, db, config, getGenerator, emitHandler);
...
// Connects a Http RequestListener over a request parser and an database object
// to an callback
function httpHandler(parse, db, config, getGen, callback) {
    return function(req, res) {
        ...
        function parsedRequest(err, xapiRequest) {
            if (err) {
                ...
                var generator = getGen(contentType, config.generator, req.url);
