...
var log = injector.require('log4js')().getLogger('validator');
...
module.exports = {
    'validate' : function(xapiRequest,callback){
        ...
        if (xapiRequest.bbox && (xapiRequest.bbox.left < -180 || xapiRequest.bbox.right > 180 || xapiRequest.bbox.top > 90 || xapiRequest.bbox.bottom < -90)){
            error.code = 400;
        ...
        if (xapiRequest.bbox && (xapiRequest.bbox.left > xapiRequest.bbox.right || xapiRequest.bbox.top < xapiRequest.bbox.bottom)){
            error.code = 400;
        ...
        if (xapiRequest.tag && xapiRequest.child && xapiRequest.child.attribute ==='tag' && xapiRequest.child.has === false){
            error.code = 204;
        ...
        if (xapiRequest.tag && xapiRequest.child && xapiRequest.child.attribute === 'tag' && xapiRequest.child.has === true){
        ...
        if (xapiRequest.object === 'node' && xapiRequest.child && xapiRequest.child.attribute === 'node'){
            error.code = 204;
            log.warn('Warning: validating request returns an empty string', error);
            callback(error,null);
            return;
        ...
        if (xapiRequest.object === 'way' && xapiRequest.child && xapiRequest.child.attribute === 'way'){
            error.code = 204;
            log.warn('Warning: validating request returns an empty string', error);
            callback(error,null);
            return;
        ...
        //way object and child predicate attribute node - ignore child predicate, pass on the rest of the request
        //according to specification ways have at least 2 and at most 2000 nodes
        if (xapiRequest.object == 'way' && xapiRequest.child && xapiRequest.child.attribute === 'node'){
            xapiRequestModified.object = xapiRequest.object;
            xapiRequestModified.bbox = xapiRequest.bbox;
            xapiRequestModified.tag = xapiRequest.tag;
            callback(null,xapiRequestModified);
