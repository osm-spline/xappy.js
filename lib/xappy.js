...
function Xappy(config) {
    ...
    var parsedHandler =
        getXappyRequestHandler(db, getGeneratorSelector(config.copyright), getEmitterHandler);
...
function getGeneratorSelector(copyright) {
    return function generatorSelector(contentType) {
        ...
        if (json_hit == -1 && xml_hit == -1) { throw {'message': "Invalid Content-Type", 'code':417}; }
 ...
 *
 *               The generated callback takes the emitter or an error as result. Its task is to bind handlers to the event.
 *
 */
function getEmitterHandler(res, generator) {
    return function emitterHanlder(err, emitter) {
