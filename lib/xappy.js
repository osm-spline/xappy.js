...
function Xapi(config) {
    ...
    var parsedHandler = getXapiRequestHandler(db, getGenerator(genConf), emitHandler);
...
function getGeneratorSelector(config) {
    return function generatorSelector(contentType, uri) {
        var isJson = (contentType == "application/json");
        var Generator =  isJson? JSONGenerator : XmlGenerator;

        config.uri=uri;
        config.planetDate = "SELECT planetDate from DATABASE!!!!"; // TODO XXX TODO
        config.generator = "xappy.js v0.2"; //+version.xappy;
        config.version = "0.6";
...
/**
 * @description: Handles xapiRequest: Returns error to the stream or creates a
 *               OutputGenerator and runs the request against db.
 */
