...
function getHttpHandler(parse, callback) {
    return function httpHandler(req, res) {
        ...
        parse(pathname, function (err, xappyReq) {
            ...
            if (err) {
                callback(req, res, content, err, xappyReq);
            } else {
...
function getXappyRequestHandler(db, getGen, callback) {
    return function xappyRequestHandler(req, res, contentType, err, xappyRequest) {
        if (err) {
            ...
            db.executeRequest(xappyRequest, callback(req, res, generator));
        }
    };
}
 ...
 *
*/
function getEmitterHandler(req, res, generator) {
    return function emitterHandler(err, emitter) {
        ...
        if (err) {
            ...
            req.on('close',function (err) {
                log.warn("Request cancled by: " + err);
                emitter.close();
            });
