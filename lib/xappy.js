...
var sanatizeUrl = function sanatizeUrl(xrs, next){
        log.info('recieved request ' + xrs.req.url);
...
var getGenerator = function getGenerator(xrs, next){
    ...
    try {
        ...
        if(!e.code){
            next({ code: 500, msg: 'Error evaluate contenttyp'});
        } else {
            next(e);
        }
    }
...
var writeRes = function writeRes(xrs, next) {
    ...
    req.once('close',function() {
        log.info('Connection was closed by remote host ' + req.connection.remoteAddress);
